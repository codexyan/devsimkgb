...
export default async function PanduanPage() {
  ...
  return (
    <>
      {/* Pilihan peran dipasang sebelum isi tergambar agar bagian yang tidak relevan tidak sempat tampil.
          Tanpa JavaScript skrip ini tidak berjalan dan seluruh panduan tampil. */}
      <script
        dangerouslySetInnerHTML={{
          __html: `try{var h=document.documentElement,s=${JSON.stringify([...PERAN.map((p) => p.id), SEMUA])},p=new URLSearchParams(location.search).get("peran"),t=location.hash.slice(1);h.dataset.panduan="siap";if(s.indexOf(p)>=0)h.dataset.peran=p;else if(t)h.dataset.peran="${SEMUA}"}catch(e){}`,
        }}
      />

      <section className="pub-navy pub-hero pg-hero" aria-labelledby="pg-judul">
        <LatarNavy />
        <div className="pub-container">
          <header className="pg-kepala">
            <p className="pub-eyebrow pg-atas masuk">Panduan KGB untuk UPT dan Kanwil</p>
            <h1 id="pg-judul" className="pub-h1 pg-judul">
              <Kata teks="Mulai dari peran Anda" />
            </h1>
            <p className="pub-lead pg-lead masuk" style={{ "--d": 180 } as React.CSSProperties}>
              Pilih peran. Panduan hanya menampilkan langkah yang perlu Anda baca, berurutan dari awal sampai selesai.
            </p>
          </header>
      ...
      <div className="pub-container pg" id="panduan-isi">
        <p className="pg-petunjuk">Pilih peran di atas untuk mulai membaca panduan.</p>

        <div className="pg-wadah">
          <PeranAktif />
