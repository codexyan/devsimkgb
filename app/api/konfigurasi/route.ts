...
export async function GET() {
  ...
  const config = await prisma.konfigurasiKanwil.findUnique({
    where: { id: "default" },
  });
...
export async function PATCH(req: Request) {
  ...
  // Sanitasi field operasional (dengan batas aman)
  ...
  // H2 (mendesak) harus <= H1 (awal); tukar bila terbalik
  ...
  // Upsert: baris konfigurasi dibuat otomatis bila belum ada (mis. database baru)
  const data = {
    namaKepala,
    nipKepala,
  ...
  const config = await prisma.konfigurasiKanwil.upsert({
    where: { id: "default" },
    update: data,
