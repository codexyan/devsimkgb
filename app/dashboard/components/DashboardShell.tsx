...
export default function DashboardShell({ nama, nip, role, sesiTimeoutMenit = 60, batasInputSdm, children }: DashboardShellProps) {
  ...
  // Escape pada peringatan sesi berarti tetap masuk.
  const refPeringatan = useDialogModal(showIdleWarning, resetIdle);
        ...
        <>
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
            <div
              ...
              role="alertdialog"
              aria-modal="true"
              aria-labelledby="judul-sesi-berakhir"
              aria-describedby="isi-sesi-berakhir"
              tabIndex={-1}
              style={{ background: "var(--paper)", border: "1px solid var(--frost-border)", borderRadius: "12px", width: "100%", maxWidth: "340px", padding: "24px", display: "flex", flexDirection: "column", gap: "16px", boxShadow: "0 20px 60px rgba(9,17,53,0.18)", outline: "none" }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{ width: "38px", height: "38px", borderRadius: "9px", background: "var(--tint-amber-bg)", border: "1px solid var(--tint-amber-ln)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="#b45309">
                    <path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/>
                  </svg>
                </div>
                <div>
                  <h2 id="judul-sesi-berakhir" style={{ fontSize: "13px", fontWeight: 600, color: "var(--midnight-ink)", margin: 0 }}>Sesi Akan Berakhir</h2>
                  <p style={{ fontSize: "11px", color: "var(--mist)", margin: 0 }}>Tidak ada aktivitas terdeteksi</p>
                </div>
              </div>
              <p id="isi-sesi-berakhir" style={{ fontSize: "12px", color: "var(--slate)", lineHeight: 1.5, margin: 0 }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <button onClick={resetIdle} autoFocus className="cb-btn" style={{ flex: 1, fontSize: "12px", padding: "9px" }}>
                  Tetap Masuk
                </button>
              </div>
            </div>
