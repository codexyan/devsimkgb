...
/* Dipetakan dari nilai `aksi` yang BENAR-BENAR dipakai logAudit di codebase —
   jangan tambah entri yang tidak ada, agar dropdown filter selalu relevan. */
const AKSI_CONFIG: Record<string, { label: string; bg: string; color: string; icon: React.ReactNode }> = {
  ...
  generate_surat:           { label: "Generate Surat",      bg: "var(--tint-green-bg)",  color: "var(--st-green)",  icon: I.doc },
  upload_sk:                { label: "Upload SK",           bg: "var(--tint-green-bg)",  color: "var(--st-green)",  icon: I.up },
  ...
  edit_pegawai:             { label: "Edit Pegawai",        bg: "var(--sub)",            color: "var(--dt3)",       icon: I.edit },
  hapus_pegawai:            { label: "Hapus Pegawai",       bg: "var(--tint-red-bg)",    color: "var(--st-red)",    icon: I.trash },
  import_pegawai:           { label: "Import Pegawai",      bg: "var(--tint-navy)",      color: "var(--dtn)",       icon: I.import },
  ...
  fix_arsip:                { label: "Perbaikan Arsip",     bg: "var(--tint-amber-bg)",  color: "var(--st-amber)",  icon: I.edit },
  edit_profil:              { label: "Edit Profil",         bg: "var(--sub)",            color: "var(--dt3)",       icon: I.edit },
          ...
          <div className="adm-modal" style={{ maxWidth: "24rem" }} onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="w-10 h-10 rounded-full flex items-center justify-center mb-3" style={{ background: "var(--tint-red-bg)" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--st-red)" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>
              </div>
              <h2 className="text-sm font-semibold mb-1" style={{ color: "var(--dtn)" }}>Hapus {selected.size} Entri Log?</h2>
      ...
      <form onSubmit={handleFilterSubmit} className="bg-white rounded-2xl p-4 space-y-3" style={{ border: "0.5px solid var(--ln1)" }}>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex flex-col gap-1 min-w-35">
            <label className="text-xs font-medium" style={{ color: "var(--dt4)" }}>Jenis Aktivitas</label>
            <select value={filterAksi} onChange={(e) => setFilterAksi(e.target.value)} className="adm-input" style={{ padding: "8px 10px" }}>
              <option value="">Semua</option>
              {AKSI_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1 min-w-30">
            <label className="text-xs font-medium" style={{ color: "var(--dt4)" }}>User</label>
            <input value={filterUser} onChange={(e) => setFilterUser(e.target.value)} placeholder="Nama user…" className="adm-input" style={{ padding: "8px 10px" }} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium" style={{ color: "var(--dt4)" }}>Dari</label>
            <input type="date" value={filterDari} onChange={(e) => { setFilterDari(e.target.value); setRange("all"); }} className="adm-input" style={{ padding: "8px 10px" }} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium" style={{ color: "var(--dt4)" }}>Sampai</label>
            <input type="date" value={filterSampai} onChange={(e) => { setFilterSampai(e.target.value); setRange("all"); }} className="adm-input" style={{ padding: "8px 10px" }} />
          </div>
          <div className="flex flex-col gap-1 flex-1 min-w-40">
            <label className="text-xs font-medium" style={{ color: "var(--dt4)" }}>Cari</label>
            <input value={filterQ} onChange={(e) => setFilterQ(e.target.value)} placeholder="Nama / detail aksi…" className="adm-input" style={{ padding: "8px 10px" }} />
          </div>
          <div className="flex gap-2">
            <button type="submit" className="px-4 py-2 rounded-xl text-xs font-semibold" style={{ background: "var(--navy-solid)", color: "#fff" }}>Filter</button>
            <button type="button" onClick={resetFilters} className="px-4 py-2 rounded-xl text-xs font-medium" style={{ background: "var(--sub)", color: "var(--dt3)", border: "0.5px solid var(--ln0)" }}>Reset</button>
          </div>
        </div>
      </form>
      ...
      <div className="bg-white rounded-2xl overflow-hidden" style={{ border: "0.5px solid var(--ln1)" }}>
            ...
            <table className="w-full">
              <thead>
                <tr style={{ background: "var(--sub)", borderBottom: "0.5px solid var(--ln1)" }}>
                  <th className="px-4 py-3 text-left"><input type="checkbox" checked={selected.size === data.data.length && data.data.length > 0} onChange={toggleAll} className="rounded" /></th>
                  {["Waktu", "User", "Aksi", "Detail", "Target", "IP"].map((h) => (
                ...
                {data.data.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: i < data.data.length - 1 ? "0.5px solid var(--ln2)" : "none", background: isSelected ? "var(--tint-blue-bg)" : "transparent" }}>
                      <td className="px-4 py-3"><input type="checkbox" checked={isSelected} onChange={() => toggleSelect(item.id)} className="rounded" /></td>
        ...
        {data && data.totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: "0.5px solid var(--ln2)" }}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="text-xs px-3 py-1.5 rounded-lg disabled:opacity-30 transition" style={{ background: "var(--sub)", color: "var(--dtn)", border: "0.5px solid var(--ln0)" }}>← Sebelumnya</button>
              <span className="text-xs font-medium" style={{ color: "var(--dt3)" }}>{page} / {data.totalPages}</span>
              <button onClick={() => setPage((p) => Math.min(data.totalPages, p + 1))} disabled={page >= data.totalPages} className="text-xs px-3 py-1.5 rounded-lg disabled:opacity-30 transition" style={{ background: "var(--sub)", color: "var(--dtn)", border: "0.5px solid var(--ln0)" }}>Selanjutnya →</button>
            </div>
          </div>
        )}
      </div>
