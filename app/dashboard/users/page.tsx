...
export default function UsersPage() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      setShowTambah(false); setShowReset(null); setShowHapus(null); setShowTolak(null);
    }
  ...
  async function fetchRequests() {
    ...
    if (res.ok) setRequests(await res.json() as any);
    setLoadingReq(false);
  }
  ...
  async function handleReset() {
    ...
    setSubmitting(false);
    if (!res.ok) { setError(data.error); return; }
    setSuccess(`Password ${showReset.nama} berhasil direset!`);
  ...
  async function handleHapus() {
    ...
    const data = await res.json() as any;
    setSubmitting(false);
    if (res.status === 409 && data.needsReassign) { setReassignCounts(data.counts); return; }
    if (!res.ok) { setError(data.error); return; }
    setSuccess(data.reassigned ? `User ${showHapus.nama} dihapus. ${data.reassigned} data dialihkan.` : `User ${showHapus.nama} berhasil dihapus!`);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-base font-semibold leading-tight" style={{ color: "var(--dtn)" }}>Manajemen Pengguna</h1>
          <p className="text-xs" style={{ color: "var(--dt4)" }}>Buat &amp; kelola akun · tinjau permintaan perubahan profil</p>
        </div>
        {tab === "users" && (
      ...
      {tab === "users" && !loadingUsers && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
          {STAT_ORDER.map((r) => {
            ...
            return (
              <button key={r} onClick={() => setRoleFilter(active ? "" : r)} className={`adm-stat clickable ${active ? "active" : ""}`} style={{ textAlign: "left" }}>
      ...
      <div className="flex gap-1 p-1 rounded-xl w-fit" style={{ background: "var(--ln2)" }}>
        {([{ key: "users", label: "Akun Pengguna" }, { key: "requests", label: "Permintaan Profil", badge: requests.length }] as { key: "users" | "requests"; label: string; badge?: number }[]).map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className="flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-medium transition"
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative flex-1 min-w-52">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nama atau NIP…" className="adm-input" style={{ paddingLeft: "34px" }} />
            </div>
            {roleFilter && (
                ...
                <table className="w-full">
                  <thead>
                    <tr style={{ background: "var(--sub)", borderBottom: "0.5px solid var(--ln1)" }}>
                      {["Nama", "NIP", "Role", "Dibuat", ""].map((h) => (
                        <th key={h} className={`text-left px-5 py-3 text-xs font-semibold ${!h ? "text-right" : ""}`} style={{ color: "var(--dt4)" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <tr key={user.id} className="u-row" style={{ borderBottom: i < filteredUsers.length - 1 ? "0.5px solid var(--ln2)" : "none" }}>
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => { setShowReset(user); setFormReset(""); setError(""); }} title="Reset password" className="w-8 h-8 rounded-lg flex items-center justify-center transition" style={{ background: "var(--tint-navy)", color: "var(--dtn)", border: "0.5px solid var(--ln0)" }}>
                              ...
                              <button onClick={() => { setShowHapus(user); setReassignCounts(null); setReassignTo(""); setError(""); }} title="Hapus user" className="w-8 h-8 rounded-lg flex items-center justify-center transition" style={{ background: "var(--tint-red-bg)", color: "var(--st-red)", border: "0.5px solid var(--tint-red-ln)" }}>
                ...
                <div key={req.id} className="px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    {req.nama && req.nama !== req.user.nama && (
                      <div className="flex items-center gap-2 text-xs"><span style={{ color: "var(--dt4)" }}>Nama:</span><span className="line-through" style={{ color: "var(--dt6)" }}>{req.user.nama}</span><span>→</span><span className="font-medium" style={{ color: "var(--dtn)" }}>{req.nama}</span></div>
                    )}
                    {req.jabatan !== null && req.jabatan !== req.user.jabatan && (
                      <div className="flex items-center gap-2 text-xs"><span style={{ color: "var(--dt4)" }}>Jabatan:</span><span className="line-through" style={{ color: "var(--dt6)" }}>{req.user.jabatan || "-"}</span><span>→</span><span className="font-medium" style={{ color: "var(--dtn)" }}>{req.jabatan || "-"}</span></div>
                    )}
                    {req.email !== null && req.email !== req.user.email && (
                      <div className="flex items-center gap-2 text-xs"><span style={{ color: "var(--dt4)" }}>Email:</span><span className="line-through" style={{ color: "var(--dt6)" }}>{req.user.email || "-"}</span><span>→</span><span className="font-medium" style={{ color: "var(--dtn)" }}>{req.email || "-"}</span></div>
                    )}
      ...
      {/* Modal Tambah User */}
      {showTambah && (
        <div className="adm-overlay" onClick={() => setShowTambah(false)}>
          <div className="adm-modal" style={{ maxWidth: "26rem" }} onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: "0.5px solid var(--ln2)", background: "var(--sub)" }}>
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-sm font-semibold leading-tight" style={{ color: "var(--dtn)" }}>Tambah Pengguna Baru</h2>
                <p className="text-xs" style={{ color: "var(--dt4)" }}>Akun login untuk tim pengelola SIM-KGB</p>
              </div>
              <button onClick={() => setShowTambah(false)} className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: "var(--ln2)", color: "var(--dt3)" }}>
            ...
            <div className="p-5 space-y-3.5">
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--dt2)" }}>NIP</label>
                <input
                ...
                />
                <p className="text-xs mt-1" style={{ color: formTambah.nip.length === 18 ? "var(--st-green)" : "var(--dt5)", fontSize: "10px" }}>
                  {formTambah.nip.length}/18 digit{formTambah.nip.length === 18 ? " ✓" : ""}
                </p>
              </div>
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--dt2)" }}>Nama Lengkap</label>
                <input type="text" className={inputClass} placeholder="Nama beserta gelar" value={formTambah.nama} onChange={(e) => setFormTambah((p) => ({ ...p, nama: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--dt2)" }}>Password</label>
                <div className="relative">
                  <input type={showPwd ? "text" : "password"} className={inputClass} style={{ paddingRight: "38px" }} placeholder="Minimal 6 karakter" value={formTambah.password} onChange={(e) => setFormTambah((p) => ({ ...p, password: e.target.value }))} />
              ...
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--dt2)" }}>Role</label>
                <div className="grid grid-cols-2 gap-2">
                  {(["sdm_kgb", "sdm_hukdis", "keuangan", "superAdminCore"] as const).map((r) => {
                    const cfg = roleCfg(r);
                    const active = formTambah.role === r;
                    return (
                      <button key={r} type="button" onClick={() => setFormTambah((p) => ({ ...p, role: r }))}
            ...
            <div className="flex gap-2 px-5 py-4" style={{ borderTop: "0.5px solid var(--ln2)" }}>
              <button onClick={() => setShowTambah(false)} className="flex-1 text-xs py-2.5 rounded-xl" style={{ border: "0.5px solid var(--ln1)", color: "var(--dt4)" }}>Batal</button>
      ...
      {showReset && (
        <div className="adm-overlay" onClick={() => setShowReset(null)}>
          <div className="adm-modal" style={{ maxWidth: "24rem" }} onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <h2 className="text-sm font-semibold mb-3" style={{ color: "var(--dtn)" }}>Reset Password</h2>
              ...
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--dt2)" }}>Password Baru</label>
              <div className="relative">
                <input autoFocus type={showPwdReset ? "text" : "password"} className={inputClass} style={{ paddingRight: "38px" }} placeholder="Minimal 6 karakter" value={formReset} onChange={(e) => setFormReset(e.target.value)} />
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={() => setShowReset(null)} className="flex-1 text-xs py-2.5 rounded-xl" style={{ border: "0.5px solid var(--ln1)", color: "var(--dt4)" }}>Batal</button>
                <button onClick={handleReset} disabled={submitting} className="flex-1 text-xs py-2.5 rounded-xl font-semibold text-white disabled:opacity-50" style={{ background: "var(--navy-solid)" }}>{submitting ? "Mereset…" : "Reset Password"}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showTolak && (
        <div className="adm-overlay" onClick={() => setShowTolak(null)}>
          <div className="adm-modal" style={{ maxWidth: "24rem" }} onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="w-10 h-10 rounded-full flex items-center justify-center mb-3" style={{ background: "var(--tint-red-bg)" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--st-red)" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              </div>
              <h2 className="text-sm font-semibold mb-1" style={{ color: "var(--dtn)" }}>Tolak Permintaan?</h2>
              ...
              <label className="block text-xs font-medium mb-1" style={{ color: "var(--dt2)" }}>Alasan penolakan <span style={{ color: "var(--dt5)" }}>(opsional)</span></label>
              <textarea rows={3} className="adm-input resize-none" placeholder="Contoh: Data tidak sesuai, harap lengkapi terlebih dahulu" value={alasanTolak} onChange={(e) => setAlasanTolak(e.target.value)} />
              {error && <p className="text-xs mt-3" style={{ color: "var(--st-red)" }}>{error}</p>}
              <div className="flex gap-2 mt-5">
                <button onClick={() => setShowTolak(null)} className="flex-1 text-xs py-2.5 rounded-xl" style={{ border: "0.5px solid var(--ln1)", color: "var(--dt4)" }}>Batal</button>
      ...
      {showHapus && (
        <div className="adm-overlay" onClick={() => { setShowHapus(null); setReassignCounts(null); setReassignTo(""); setError(""); }}>
          <div className="adm-modal" style={{ maxWidth: "24rem" }} onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="w-10 h-10 rounded-full flex items-center justify-center mb-3" style={{ background: "var(--tint-red-bg)" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--st-red)" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
              </div>
              {!reassignCounts ? (
                <>
                  <h2 className="text-sm font-semibold mb-1" style={{ color: "var(--dtn)" }}>Hapus Pengguna?</h2>
                ...
                <>
                  <h2 className="text-sm font-semibold mb-1" style={{ color: "var(--dtn)" }}>Alihkan Data Sebelum Menghapus</h2>
                  ...
                  <div className="mb-4">
                    <label className="block text-xs font-medium mb-1" style={{ color: "var(--dt2)" }}>Alihkan data ke <span style={{ color: "var(--st-red)" }}>*</span></label>
                    ...
                    <p className="text-xs mt-1.5" style={{ color: "var(--dt4)" }}>Semua data akan dipindahkan ke user yang dipilih.</p>
                  </div>
                </>
              )}
              {error && <p className="text-xs mb-3" style={{ color: "var(--st-red)" }}>{error}</p>}
              <div className="flex gap-2">
                <button onClick={() => { setShowHapus(null); setReassignCounts(null); setReassignTo(""); setError(""); }} className="flex-1 text-xs py-2.5 rounded-xl" style={{ border: "0.5px solid var(--ln1)", color: "var(--dt4)" }}>Batal</button>
                <button onClick={handleHapus} disabled={submitting || (!!reassignCounts && !reassignTo)} className="flex-1 text-xs py-2.5 rounded-xl font-semibold text-white disabled:opacity-40" style={{ background: "var(--red-solid)" }}>{submitting ? "Memproses…" : reassignCounts ? "Alihkan & Hapus" : "Ya, Hapus"}</button>
              </div>
            </div>
          </div>
        </div>
